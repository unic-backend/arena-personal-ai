...
def sans_fait_enregistre(monkeypatch):
    ...
    monkeypatch.setattr(main.memory, "get_fact", lambda cle: "Saer" if cle == "owner" else None)


@pytest.fixture
def date_fixee(monkeypatch):
    def _fixer(annee, mois, jour):
        monkeypatch.setattr(main, "date_du_jour", lambda: date(annee, mois, jour))
    return _fixer
...
def test_aucun_fait_date_n_est_ecrit_en_dur(sans_fait_enregistre, affirmation):
    assert affirmation not in main.get_arena_system_prompt()
...
def test_le_code_source_ne_contient_plus_ces_valeurs():
    ...
    source = (main.BASE_DIR / "apps" / "backend" / "main.py").read_text(encoding="utf-8")
    ligne_de_code = [
...
def test_la_date_est_lue_sur_la_machine_pas_ecrite(sans_fait_enregistre, date_fixee):
    ...
    prompt = main.get_arena_system_prompt()
...
def test_le_proprietaire_est_nomme(sans_fait_enregistre):
    assert "Saer" in main.get_arena_system_prompt()
...
def test_un_fait_enregistre_apparait_avec_sa_reserve(monkeypatch):
    ...
    monkeypatch.setattr(main.memory, "get_fact", lambda cle: valeurs.get(cle))
