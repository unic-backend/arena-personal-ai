...
class MemoryEngine:
    ...
    def _local_context(self, user_id: str, conversation_id: str, query: str) -> MemoryContext:
        ...
        with closing(self._connect()) as db:
            ...
            rows = db.execute(
                "SELECT id,kind,content,source,created,conversation_id,index_space FROM autonomous_documents "
                "WHERE user_id=? " + f"AND (kind='user_fact' OR ({conditions})) "
                "ORDER BY (conversation_id=?) DESC,(kind='user_fact') DESC,created DESC LIMIT 16",
                [user_id, *words, conversation_id],
            ).fetchall()
        memories = [{**dict(row), "provenance": "sqlite_lexical"} for row in rows]
        warnings: list[str] = []
        local_only = False
        if user_id == "owner" and self.legacy_memory is not None:
            try:
                ...
                memories.extend({"kind": item.souvenir.nature.value, "content": item.souvenir.contenu,
                                 "source": item.souvenir.source, "provenance": "legacy_memory",
                                 "created": 0.0, "conversation_id": "legacy"}
    ...
    async def context(self, user_id: str, conversation_id: str, query: str,
                      semantic: bool = True) -> MemoryContext:
        try:
        ...
        try:
            ...
            result.memories = (hits + [row for row in result.memories if row["content"] not in seen])[:20]
        except Exception:
    ...
    def _query_vectors(self, user_id: str, vector: list[float]) -> list[dict[str, Any]]:
        response = self._chroma().query(query_embeddings=[vector], n_results=20,
        ...
        with closing(self._connect()) as db:
            ...
            for index, document_id in enumerate(ids):
                row = db.execute(
                    "SELECT id,kind,content,source,created,conversation_id,index_space "
                    "FROM autonomous_documents WHERE id=? AND user_id=?",
                    (document_id, user_id),
                ).fetchone()
                if row:
                    distance = float(distances[index]) if index < len(distances) else 1.0
                    result.append({**dict(row), "provenance": "semantic",
        ...
        try:
            response = await self.ai.complete([
                {"role": "system", "content": (
                    ...
                    'preferences, identite, contraintes, decisions, projets et taches durables. Ignore salutations, '
                    'questions, hypotheses, citations de tiers, bruit temporaire et faits provenant seulement de assistant. '
                    'Retourne JSON {"facts":[{"evidence":"citation exacte du message utilisateur"}]}. '
                    'Si rien ne convient : {"facts":[]}.')},
                {"role": "user", "content": json.dumps({"utilisateur": job["question"]}, ensure_ascii=False)},
            ], json_mode=True)
    ...
    async def drain(self) -> None:
        ...
        async with self._drain_lock:
            try:
                ...
                logger.warning("Lot memoire reporte; les donnees sources sont conservees.")
